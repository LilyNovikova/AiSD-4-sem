...
public:
	...
	MapNode* find(ValT val_to_find)
	{
		if (!this) return nullptr;
		if (is_empty(val_to_find)) throw invalid_argument("Value to find is empty");
		MapNode to_find = MapNode(NULL, val_to_find);
		if (to_find == *this) return this;
		else
			if (to_find > *this && right)
				return right->find(val_to_find);
			else
				if (to_find < *this && left)
					return left->find(val_to_find);
		return nullptr;
	}
